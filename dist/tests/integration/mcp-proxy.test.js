...
describe('MCP Proxy Integration Tests', () => {
    ...
    describe('Bearer Token Extraction and Validation', () => {
        ...
        it('should reject invalid Bearer token', async () => {
            await expect(jwtService.verifyToken('invalid-token')).rejects.toThrow('JWT verification failed');
        });
    ...
    describe('Request Forwarding with Error Handling', () => {
        ...
        it('should handle request timeout', async () => {
            ...
            // Mock slow response
            const mockFetch = vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(() => {
                resolve({
                    ok: true,
                    status: 200,
                    statusText: 'OK',
            ...
            global.fetch = mockFetch;
            await expect(proxyService.forwardRequest(testServerId, proxyRequest)).rejects.toThrow('UPSTREAM_REQUEST_FAILED');
        });
    });
    describe('Resource Identifier Routing (RFC 8707)', () => {
        it('should route request by resource identifier', async () => {
            ...
            global.fetch = mockFetch;
            const response = await proxyService.forwardRequestByResource(resourceIdentifier, proxyRequest);
            expect(response.status).toBe(200);
            expect(mockRegistry.getServerByResource).toHaveBeenCalledWith(resourceIdentifier, testTenantId);
        });
    });
});
