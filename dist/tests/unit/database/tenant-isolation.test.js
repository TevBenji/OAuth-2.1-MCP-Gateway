...
describe('Tenant Isolation and Database Layer', () => {
    describe('TenantIsolationService', () => {
        let tenantIsolationService;
        beforeEach(() => {
            tenantIsolationService = new TenantIsolationService(mockD1Database);
            vi.clearAllMocks();
        });
        ...
        it('should detect existing tenant filters', () => {
            ...
            const tenantId = 'test-tenant-123';
            const hasFilter = tenantIsolationService.hasTenantFilter(queryWithFilter, tenantId);
        ...
        it('should validate resource access correctly', async () => {
            ...
            const hasAccess = await tenantIsolationService.hasResourceAccess('tenant-123', 'client', 'client-456');
            expect(hasAccess).toBe(true);
            expect(mockD1Database.prepare).toHaveBeenCalledWith('SELECT 1 FROM oauth_clients WHERE id = ? AND tenant_id = ?');
        });
        it('should prevent cross-tenant access', async () => {
            const isValid = await tenantIsolationService.validateCrossTenantAccess('tenant-123', 'tenant-456');
            expect(isValid).toBe(false);
        });
        it('should allow same-tenant access', async () => {
            const isValid = await tenantIsolationService.validateCrossTenantAccess('tenant-123', 'tenant-123');
    ...
    describe('TenantConfigService', () => {
        ...
        it('should create a new tenant with default configuration', async () => {
            const newTenant = await tenantConfigService.createTenant('new-tenant-id', 'New Tenant', 'A new test tenant');
        ...
        it('should update tenant configuration', async () => {
            const updatedConfig = await tenantConfigService.updateTenantConfig('test-tenant', { compliance_tier: 'enterprise' });
        ...
        it('should update compliance tier and adjust limits accordingly', async () => {
            const updatedConfig = await tenantConfigService.updateComplianceTier('test-tenant', 'enterprise');
            expect(updatedConfig).toBeDefined();
            expect(updatedConfig?.compliance_tier).toBe('enterprise');
            if (updatedConfig) {
        ...
        it('should check if tenant limits are exceeded', async () => {
            const isExceeded = await tenantConfigService.isLimitExceeded('test-tenant', 'max_clients');
    ...
    describe('DatabaseManager', () => {
        let dbManager;
        beforeEach(() => {
            dbManager = new DatabaseManager(mockD1Database);
        });
