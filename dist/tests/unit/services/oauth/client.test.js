...
describe('ClientService', () => {
    ...
    describe('getClient', () => {
        ...
        it('should retrieve and parse client data correctly', async () => {
            ...
            expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringContaining('SELECT * FROM oauth_clients'));
    ...
    describe('validateClient', () => {
        ...
        it('should validate client with correct secret', async () => {
            ...
            const result = await clientService.validateClient('mcp_client_test123', 'mcp_secret_test456', 'test-tenant');
        ...
        it('should reject client with incorrect secret', async () => {
            ...
            const result = await clientService.validateClient('mcp_client_test123', 'wrong_secret', 'test-tenant');
    ...
    describe('isValidRedirectUri', () => {
        ...
        it('should validate registered redirect URI', async () => {
            ...
            const result = await clientService.isValidRedirectUri('mcp_client_test123', 'https://example.com/callback', 'test-tenant');
            expect(result).toBe(true);
        });
        it('should reject unregistered redirect URI', async () => {
            ...
            const result = await clientService.isValidRedirectUri('mcp_client_test123', 'https://malicious.com/callback', 'test-tenant');
            expect(result).toBe(false);
        });
        it('should reject for non-existent client', async () => {
            mockStatement.first.mockResolvedValue(null);
            const result = await clientService.isValidRedirectUri('non-existent', 'https://example.com/callback', 'test-tenant');
            expect(result).toBe(false);
        });
    });
    describe('deleteClient', () => {
        it('should delete existing client', async () => {
            ...
            expect(result).toBe(true);
            expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM oauth_clients WHERE client_id = ? AND tenant_id = ?'));
            expect(mockStatement.bind).toHaveBeenCalledWith('mcp_client_test123', 'test-tenant');
        });
