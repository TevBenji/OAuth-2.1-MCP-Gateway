...
export class APIKeyManager {
    keys = new Map(); // In-memory storage for development
    keyPrefixMap = new Map(); // Maps prefix to key ID
    keyIdToHash = new Map(); // Maps key ID to hashed key
    /**
     ...
     */
    async scheduleAutomatedRotation(keyId, intervalDays = 90) {
     ...
     */
    async performAutomatedRotations(tenantId) {
        const now = new Date();
        const keysToRotate = [];
        for (const key of this.keys.values()) {
     ...
     */
    scheduleKeyDeletion(keyId, delayMs) {
     ...
     */
    async notifyAboutRotation(oldKey, newKey) {
