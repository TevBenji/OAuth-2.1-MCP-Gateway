...
export class AuditService {
     ...
     */
    async createLogEntry(tenantId, event, action, success, options) {
        const logEntry = {
    ...
    async storeLogEntry(entry) {
        ...
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(entry.id, entry.timestamp, entry.event, entry.action, entry.success ? 1 : 0, entry.tenantId, entry.userId || null, entry.clientId || null, entry.resourceId || null, entry.resourceType || null, entry.ipAddress || null, entry.userAgent || null, entry.sessionId || null, entry.requestId || null, detailsJson, complianceTagsJson, entry.severity, entry.source).run();
        }
        catch (error) {
    ...
    `;
        ...
        const result = await this.db.prepare(baseQuery).bind(...params).all();
    ...
    convertToXML(entries) {
        ...
        for (const entry of entries) {
            ...
            if (entry.userId)
                xml += `    <userId>${this.escapeXMLField(entry.userId)}</userId>\n`;
            if (entry.clientId)
                xml += `    <clientId>${this.escapeXMLField(entry.clientId)}</clientId>\n`;
            if (entry.resourceId)
                xml += `    <resourceId>${this.escapeXMLField(entry.resourceId)}</resourceId>\n`;
            if (entry.resourceType)
                xml += `    <resourceType>${this.escapeXMLField(entry.resourceType)}</resourceType>\n`;
            if (entry.ipAddress)
                xml += `    <ipAddress>${this.escapeXMLField(entry.ipAddress)}</ipAddress>\n`;
            if (entry.userAgent)
                xml += `    <userAgent>${this.escapeXMLField(entry.userAgent)}</userAgent>\n`;
            if (entry.sessionId)
                xml += `    <sessionId>${this.escapeXMLField(entry.sessionId)}</sessionId>\n`;
            if (entry.requestId)
                xml += `    <requestId>${this.escapeXMLField(entry.requestId)}</requestId>\n`;
        ...
        for (const [complianceTag, policy] of this.retentionPolicies.entries()) {
            if (policy.autoDelete) {
                ...
                try {
                    ...
                    await this.db.prepare(`DELETE FROM audit_logs 
             WHERE compliance_tags LIKE ? 
             AND timestamp < ?`).bind(`%${complianceTag}%`, cutoffDate.toISOString()).run();
                }
                catch (error) {
     ...
     */
    getRetentionPolicy(complianceTag) {
