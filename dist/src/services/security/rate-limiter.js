 ...
 */
import { RateLimitKeyType, RateLimitWindow, DEFAULT_RATE_LIMIT_CONFIG, } from '../../types/rate-limit';
...
export class RateLimiter {
     ...
     */
    async checkTenantLimit(tenantId, window = RateLimitWindow.MINUTE) {
     ...
     */
    async checkUserLimit(tenantId, userId, window = RateLimitWindow.MINUTE) {
     ...
     */
    async checkIPLimit(ipAddress, window = RateLimitWindow.MINUTE) {
     ...
     */
    async checkEndpointLimit(endpoint, tenantId, window = RateLimitWindow.MINUTE) {
     ...
     */
    async checkLimit(key, keyType, limits, window) {
     ...
     */
    getLimitForWindow(limits, window) {
        switch (window) {
     ...
     */
    async checkAllLimits(tenantId, userId, ipAddress) {
     ...
     */
    async detectSuspiciousActivity(key, keyType, count, limit) {
     ...
     */
    async getStatus(key, keyType, limits, window = RateLimitWindow.MINUTE) {
        const limit = this.getLimitForWindow(limits, window);
