...
export class SAMLFederationService {
     ...
     */
    buildAuthnRequest(config, requestId, redirectUri, relayState) {
...
<samlp:AuthnRequest
  ...
  <samlp:NameIDPolicy
     ...
     */
    buildAuthenticationUrl(config, requestId, redirectUri, relayState) {
        if (!config.saml_config) {
            throw new IdPError(IdPErrorCode.INVALID_CONFIGURATION, 'SAML configuration not found');
        }
        const samlConfig = config.saml_config;
    ...
    async parseSAMLResponse(samlResponse) {
        try {
            ...
            throw new IdPError(IdPErrorCode.SAML_PARSING_FAILED, `Failed to parse SAML response: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
    ...
    extractSAMLAttributes(samlXml) {
        ...
        try {
            ...
            throw new IdPError(IdPErrorCode.SAML_PARSING_FAILED, `Failed to extract SAML attributes: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
    ...
    async verifySAMLSignature(samlResponse, certificate) {
        try {
            ...
            throw new IdPError(IdPErrorCode.SAML_VALIDATION_FAILED, `SAML signature verification failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     * Process SAML callback
     */
    async processCallback(config, samlResponse, relayState) {
        if (!config.saml_config) {
            throw new IdPError(IdPErrorCode.INVALID_CONFIGURATION, 'SAML configuration not found');
        }
        const samlConfig = config.saml_config;
        ...
        if (samlConfig.want_assertions_signed || samlConfig.want_response_signed) {
            ...
            if (!signatureValid) {
                throw new IdPError(IdPErrorCode.SAML_VALIDATION_FAILED, 'SAML response signature verification failed');
            }
        }
     ...
     */
    normalizeSAMLAttributes(attributes) {
     ...
     */
    buildLogoutRequest(config, requestId, nameId, sessionIndex) {
        if (!config.saml_config) {
            throw new IdPError(IdPErrorCode.INVALID_CONFIGURATION, 'SAML configuration not found');
        }
        const samlConfig = config.saml_config;
...
<samlp:LogoutRequest
  ...
  ${sessionIndex ? `<samlp:SessionIndex>${sessionIndex}</samlp:SessionIndex>` : ''}
     ...
     */
    buildLogoutUrl(config, requestId, nameId, sessionIndex, relayState) {
