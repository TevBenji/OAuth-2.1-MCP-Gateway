 ...
 */
export class AuthorizationCodeStorageD1 {
    db;
    constructor(db) {
        this.db = db;
    }
     ...
     */
    async storeCode(code, clientId, redirectUri, userId, scopes, expiresAt, codeChallenge, challengeMethod, tenantId = 'default-tenant') {
        try {
            const now = new Date().toISOString();
            const scopeString = scopes.join(' ');
            // Use parameterized query to prevent SQL injection
            await this.db
                .prepare(`INSERT INTO ${DATABASE_CONSTANTS.TABLES.AUTHORIZATION_CODES} (
            id,
            code,
            client_id,
            redirect_uri,
            ...
            created_at
                ...
                .bind(this.generateId(), code, clientId, redirectUri, scopeString, userId, expiresAt, codeChallenge || null, challengeMethod || null, tenantId, now)
                .run();
        }
        catch (error) {
     ...
     */
    async retrieveAndDeleteCode(code, tenantId = 'default-tenant') {
        try {
            // Step 1: Retrieve the code with tenant isolation
            const result = await this.db
                .prepare(`SELECT * FROM ${DATABASE_CONSTANTS.TABLES.AUTHORIZATION_CODES}
     ...
     */
    async deleteCode(code, tenantId) {
        try {
            await this.db
                .prepare(`DELETE FROM ${DATABASE_CONSTANTS.TABLES.AUTHORIZATION_CODES}
           WHERE code = ? AND tenant_id = ?`)
                .bind(code, tenantId)
                .run();
        }
 ...
 */
export function createAuthorizationCodeStorage(db) {
