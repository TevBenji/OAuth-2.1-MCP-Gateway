 ...
 */
export class RefreshTokenStorageD1 {
    db;
    constructor(db) {
        this.db = db;
    }
     ...
     */
    async storeRefreshToken(refreshToken, accessToken, clientId, userId, scopes, expiresAt, tenantId = 'default-tenant') {
        try {
            const now = new Date().toISOString();
            const scopeString = scopes.join(' ');
            // Use parameterized query to prevent SQL injection
            await this.db
                .prepare(`INSERT INTO ${DATABASE_CONSTANTS.TABLES.REFRESH_TOKENS} (
            id,
            token,
            access_token_id,
            client_id,
            ...
            created_at
                ...
                .bind(this.generateId(), refreshToken, this.hashToken(accessToken), // Store hashed access token reference
            clientId, userId, scopeString, expiresAt, tenantId, now)
                .run();
        }
        catch (error) {
     ...
     */
    async retrieveAndDeleteRefreshToken(refreshToken, tenantId = 'default-tenant') {
        try {
            // Step 1: Retrieve the token with tenant isolation
            const result = await this.db
                .prepare(`SELECT * FROM ${DATABASE_CONSTANTS.TABLES.REFRESH_TOKENS}
     ...
     */
    async revokeRefreshToken(refreshToken, tenantId = 'default-tenant') {
        try {
            const result = await this.db
                .prepare(`DELETE FROM ${DATABASE_CONSTANTS.TABLES.REFRESH_TOKENS}
           WHERE token = ? AND tenant_id = ?`)
                .bind(refreshToken, tenantId)
                .run();
     ...
     */
    async deleteRefreshToken(token, tenantId) {
        try {
            await this.db
                .prepare(`DELETE FROM ${DATABASE_CONSTANTS.TABLES.REFRESH_TOKENS}
           WHERE token = ? AND tenant_id = ?`)
     ...
     */
    async revokeUserRefreshTokens(userId, tenantId = 'default-tenant') {
        try {
            const result = await this.db
                .prepare(`DELETE FROM ${DATABASE_CONSTANTS.TABLES.REFRESH_TOKENS}
 ...
 */
export function createRefreshTokenStorage(db) {
