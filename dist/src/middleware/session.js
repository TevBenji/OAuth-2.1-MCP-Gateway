...
export function sessionMiddleware(sessionManager) {
    return async (c, next) => {
        try {
            ...
            if (!sessionId) {
                throw new MCPError('MISSING_SESSION', 'Session ID not found in request', 401);
            }
            ...
            if (!validationResult.valid) {
                ...
                throw new MCPError(error.code, error.message, statusCode, error.details);
            }
 ...
 */
export function optionalSessionMiddleware(sessionManager) {
    return async (c, next) => {
        try {
            const sessionId = extractSessionId(c);
            if (sessionId) {
 ...
 */
export function setSessionCookie(c, sessionId, maxAge, options) {
    const cookieOptions = {
 ...
 */
export function clearSessionCookie(c, options) {
