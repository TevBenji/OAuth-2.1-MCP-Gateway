 ...
 */
function setRateLimitHeaders(c, result) {
 ...
 */
export function rateLimitMiddleware(rateLimiter, window) {
    return async (c, next) => {
        try {
            ...
            const result = await rateLimiter.checkAllLimits(mcpContext.tenant_id, mcpContext.user_id, ipAddress);
 ...
 */
export function ipRateLimitMiddleware(rateLimiter, window) {
    return async (c, next) => {
        try {
            const ipAddress = extractIPAddress(c);
            // Check IP rate limit
 ...
 */
export function tenantRateLimitMiddleware(rateLimiter, window) {
    return async (c, next) => {
        try {
            const mcpContext = c.get('mcpContext');
            if (!mcpContext) {
 ...
 */
export function userRateLimitMiddleware(rateLimiter, window) {
    return async (c, next) => {
        try {
            const mcpContext = c.get('mcpContext');
            if (!mcpContext) {
            ...
            const result = await rateLimiter.checkUserLimit(mcpContext.tenant_id, mcpContext.user_id, window);
            // Set rate limit headers
            setRateLimitHeaders(c, result);
            if (!result.allowed) {
                return c.json({
 ...
 */
export function endpointRateLimitMiddleware(rateLimiter, endpoint, window) {
    return async (c, next) => {
        try {
            const mcpContext = c.get('mcpContext');
            if (!mcpContext) {
