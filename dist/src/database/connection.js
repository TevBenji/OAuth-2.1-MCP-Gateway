...
export class DatabaseManager {
     ...
     */
    async executeWithTenant(query, params, tenantId) {
        ...
        return this.database.prepare(tenantQuery).bind(...(params || [])).all();
    }
    /**
    ...
    addTenantFilter(query, tenantId) {
        ...
        const hasTenantAwareTable = tenantAwareTables.some(table => new RegExp(`\\b${table}\\b`, 'i').test(query));
     ...
     */
    async withTenantContext(tenantId, operation) {
...
export function createDatabaseManager(database) {
