...
class MigrationRegistry {
    ...
    getAllMigrations() {
        return Array.from(this.migrations.values()).sort((a, b) => a.timestamp.localeCompare(b.timestamp));
    }
    getUnappliedMigrations(appliedMigrationIds) {
        const allMigrations = this.getAllMigrations();
        return allMigrations.filter(migration => !appliedMigrationIds.includes(migration.id));
    }
}
...
export class MigrationManager {
    db;
    registry;
    constructor(db, registry = migrationRegistry) {
        this.db = db;
    ...
    async getAppliedMigrations() {
        const result = await this.db.prepare('SELECT id FROM schema_migrations ORDER BY applied_at').all();
    ...
    async runMigrations() {
        ...
        for (const migration of unappliedMigrations) {
            ...
            try {
                ...
                await this.db.prepare('INSERT INTO schema_migrations (id, name, applied_at) VALUES (?, ?, ?)').bind(migration.id, migration.name, new Date().toISOString()).run();
    ...
    async rollbackLastMigration() {
        ...
        const appliedMigrations = await this.db.prepare('SELECT id, name FROM schema_migrations ORDER BY applied_at DESC LIMIT 1').all();
        if (appliedMigrations.results.length === 0) {
            console.log('No migrations to rollback');
            return;
        }
        const lastMigration = appliedMigrations.results[0];
        ...
        try {
            ...
            await this.db.prepare('DELETE FROM schema_migrations WHERE id = ?').bind(lastMigration.id).run();
