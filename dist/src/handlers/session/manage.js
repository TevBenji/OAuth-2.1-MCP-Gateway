 ...
 * POST /sessions
 */
export async function createSessionHandler(c) {
    try {
 ...
 */
export async function getCurrentSessionHandler(c) {
    try {
 ...
 */
export async function revokeCurrentSessionHandler(c) {
    try {
        const sessionManager = c.get('sessionManager');
        const session = c.get('session');
        if (!session) {
 ...
 * GET /sessions
 */
export async function listUserSessionsHandler(c) {
    try {
        const sessionManager = c.get('sessionManager');
        const session = c.get('session');
        if (!session) {
 ...
 */
export async function revokeSessionHandler(c) {
    try {
        const sessionManager = c.get('sessionManager');
 ...
 */
export async function revokeAllSessionsHandler(c) {
    try {
        const sessionManager = c.get('sessionManager');
        const currentSession = c.get('session');
        if (!currentSession) {
        ...
        const sessions = await sessionManager.getUserSessions(currentSession.tenant_id, currentSession.user_id);
