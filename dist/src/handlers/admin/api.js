...
app.get('/tenants/:id', async (c) => {
    ...
    try {
        const tenant = await c.env.DB.prepare(`SELECT * FROM tenants WHERE tenant_id = ?`)
            .bind(tenantId)
            .first();
        if (!tenant) {
...
app.put('/tenants/:id', async (c) => {
    ...
    try {
        await c.env.DB.prepare(`UPDATE tenants 
       ...
       WHERE tenant_id = ?`)
            .bind(name, description, compliance_tier, limits.max_users, limits.max_mcp_servers, limits.max_oauth_clients, limits.max_requests_per_minute, settings.enable_audit_logging, settings.enable_session_management, settings.enable_rate_limiting, settings.allow_custom_scopes, settings.require_mfa, tenantId)
            .run();
...
app.get('/tenants/:id/clients', async (c) => {
    const tenantId = c.req.param('id');
    try {
        const { results } = await c.env.DB.prepare(`SELECT * FROM oauth_clients WHERE tenant_id = ? ORDER BY created_at DESC`)
            .bind(tenantId)
            .all();
        return c.json(results);
    }
...
app.post('/tenants/:id/clients', async (c) => {
    ...
    try {
        ...
        const clientSecret = clientData.client_type === 'confidential'
            ...
            : undefined;
        await c.env.DB.prepare(`INSERT INTO oauth_clients (
         ...
         token_endpoint_auth_method, client_id_issued_at, created_at, updated_at
            ...
            .bind(clientId, clientSecret, tenantId, JSON.stringify(clientData.redirect_uris || []), JSON.stringify(clientData.grant_types || ['authorization_code']), JSON.stringify(clientData.response_types || ['code']), clientData.scope || '', clientData.client_name || '', clientData.client_uri || '', clientData.logo_uri || '', clientData.token_endpoint_auth_method || 'client_secret_basic', Math.floor(Date.now() / 1000))
            .run();
        return c.json({
            client_id: clientId,
