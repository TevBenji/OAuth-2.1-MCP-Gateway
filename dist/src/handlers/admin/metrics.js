...
export class MetricsCollector {
    ...
    recordHttpRequest(statusCode, latencyMs) {
    ...
    recordAuthentication(method, success) {
        this.authTotal++;
        if (success) {
            this.authSuccess++;
        }
    ...
    recordAuthorization(success, denied = false) {
        this.authzTotal++;
        if (success) {
            this.authzSuccess++;
        }
    ...
    recordTokenOperation(operation, latencyMs) {
        switch (operation) {
            case 'issue':
                this.tokensIssued++;
                break;
    ...
    // Record MCP request
    recordMCPRequest(success, latencyMs) {
        this.mcpRequests++;
    ...
    updateTenantMetrics(total, active, inactive) {
 ...
 */
export const authMetricsEndpoint = async (c) => {
    try {
        const metrics = metricsCollector.getMetrics();
        return c.json({
 ...
 */
export const tokenLatencyEndpoint = async (c) => {
    try {
        const metrics = metricsCollector.getMetrics();
        return c.json({
