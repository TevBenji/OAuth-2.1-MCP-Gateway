...
- **§2 gateway→upstream authentication** — design only. The notes say "Options, pick one: (a) private-network requirement in docs, or (b) per-upstream HMAC"; per the stop-and-ask rule I did not pick, and the choice is still open. The fixture change above does not commit the product to either.
