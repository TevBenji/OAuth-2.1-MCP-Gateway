...
export class MCPServerRegistry {
  private db: MCPServerDatabase;
  private healthCheckInterval: number;
  private healthCheckEnabled: boolean;

  constructor(
    db: MCPServerDatabase,
   ...
   */
  async registerServer(config: MCPServerConfig): Promise<MCPServerRegistryEntry> {
    ...
    const existing = await this.db.getServerByResourceIdentifier(
    ...
    if (existing) {
      throw new Error(
        `Server with resource_identifier '${config.resource_identifier}' already exists for tenant ${config.tenant_id}`
      );
    }
   ...
   */
  async getServer(serverId: string, tenantId: string): Promise<MCPServerRegistryEntry> {
   ...
   */
  async performHealthCheck(serverId: string, tenantId: string): Promise<MCPServerHealth> {
    const server = await this.getServer(serverId, tenantId);
   ...
   */
  async isServerHealthy(serverId: string, tenantId: string): Promise<boolean> {
    const server = await this.getServer(serverId, tenantId);
   ...
   */
  async validateServerForRequest(
    serverId: string,
    tenantId: string,
    ...
    if (server.config.required_scopes && server.config.required_scopes.length > 0) {
      const hasRequiredScopes = server.config.required_scopes.every(reqScope =>
        requiredScopes.some(providedScope => {
      ...
      if (!hasRequiredScopes) {
        throw new Error(
          `Insufficient scope for server ${serverId}. Required: ${server.config.required_scopes.join(', ')}`
        );
      }
    }
   ...
   */
  async recordAccess(serverId: string, tenantId: string): Promise<void> {
