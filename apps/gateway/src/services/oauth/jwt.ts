...
export interface TokenClaims {
  ...
  expiresIn?: number; // in seconds, defaults to 1 hour
  notBefore?: number; // in seconds from epoch
}
...
export class JWTService {
  private signingKey: Uint8Array | CryptoKey;
  ...
  constructor(
    ...
    issuer: string = 'oauth-mcp-gateway',
    verificationKey?: string | Uint8Array | CryptoKey
  ) {
  ...
  async createToken(claims: TokenClaims): Promise<string> {
    ...
    const token = await new SignJWT(payload)
      ...
      .setJti(jwtId)
      .sign(this.signingKey as Uint8Array);
   ...
   */
  async extractTenantId(token: string): Promise<string | null> {
    try {
...
export const JWT_CONFIG = {
  ACCESS_TOKEN_LIFETIME: 900, // 1 hour in seconds
