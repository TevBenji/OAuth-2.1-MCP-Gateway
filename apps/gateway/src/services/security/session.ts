...
export class SessionManager {
  ...
  async createSession(options: CreateSessionOptions): Promise<Session> {
    ...
    const riskAssessment = await this.assessRisk(
      { tenant_id, user_id, device_info },
      null // No previous session for new session
    );
   ...
   */
  async revokeUserSessions(tenantId: string, userId: string, reason?: string): Promise<number> {
    const sessions = await this.storage.getUserSessions(tenantId, userId);
    const activeSessions = sessions.filter(
      s => s.status === SessionStatus.ACTIVE || s.status === SessionStatus.SUSPENDED
    );
  ...
  private async assessRisk(
    ...
    let recommendedAction: 'allow' | 'challenge' | 'deny';
    if (riskScore >= 70) {
    ...
    const requiresMfa =
      this.limits.require_mfa_on_risk_elevation &&
   ...
   */
  private isDeviceMatch(device1: DeviceInfo, device2: DeviceInfo): boolean {
   ...
   */
  async getActiveSessionCount(tenantId: string, userId: string): Promise<number> {
    const sessions = await this.storage.getUserSessions(tenantId, userId);
