...
export class IdPManagerService {
  private oidcService: OIDCFederationService;
  private samlService: SAMLFederationService;
  private mappingService: AttributeMappingService;
  private roleService: RoleSynchronizationService;
  private provisioningService: JITProvisioningService;
  ...
  async initiateAuthentication(
    ...
    switch (config.type) {
      ...
      case IdPType.SAML:
        return this.samlService.buildAuthenticationUrl(
          config,
          request.state,
      ...
      default:
        throw new IdPError(
          IdPErrorCode.INVALID_CONFIGURATION,
          `Unsupported IdP type: ${config.type}`
        );
    }
  }
  ...
  async processCallback(
    ...
    switch (config.type) {
      case IdPType.OIDC:
        if (!params.code || !params.redirectUri) {
          throw new IdPError(
            IdPErrorCode.INVALID_CONFIGURATION,
            'OIDC callback requires code and redirectUri'
          );
        }

        rawClaims = await this.oidcService.processCallback(
          config,
          params.code,
          params.redirectUri,
          params.state || '',
      ...
      case IdPType.SAML:
        if (!params.samlResponse) {
          throw new IdPError(
            IdPErrorCode.INVALID_CONFIGURATION,
            'SAML callback requires samlResponse'
          );
        }

        rawClaims = await this.samlService.processCallback(
          config,
          params.samlResponse,
          params.state
        );
    ...
    const eligibility = this.provisioningService.validateUserEligibility(
      config,
      mappedAttributes
    );
    ...
    if (!eligibility.eligible) {
      throw new IdPError(
        IdPErrorCode.PROVISIONING_FAILED,
        `User not eligible for provisioning: ${eligibility.reason}`
      );
    }

    // Provision or update user
    const provisioningResult = await this.provisioningService.provisionUser(
      config,
      mappedAttributes
    );
    ...
    if (config.provisioning.sync_roles) {
      await this.roleService.synchronizeRoles(
        config.tenant_id,
        provisioningResult.user_id,
        mappedRoles,
        config
      );
    }
    ...
    return this.mappingService.buildAuthResponse(
      config,
      rawClaims,
      provisioningResult.user_id,
      provisioningResult.is_new_user
    );
  }
   ...
   */
  async getIdPConfig(tenantId: string, idpId: string): Promise<IdPConfig> {
   ...
   */
  async createIdPConfig(config: Omit<IdPConfig, 'created_at' | 'updated_at'>): Promise<IdPConfig> {
    // Validate configuration
    this.validateIdPConfig(config);

    const now = new Date();
    const fullConfig: IdPConfig = {
      ...config,
      created_at: now,
      updated_at: now,
    };
   ...
   */
  async updateIdPConfig(
    tenantId: string,
    idpId: string,
    updates: Partial<IdPConfig>
  ): Promise<IdPConfig> {
    // Get existing config
    const existingConfig = await this.getIdPConfig(tenantId, idpId);

    // Merge updates
    const updatedConfig: IdPConfig = {
      ...existingConfig,
      ...updates,
      updated_at: new Date(),
    };
   ...
   */
  async deleteIdPConfig(tenantId: string, idpId: string): Promise<void> {
    // In production:
   ...
   */
  private validateIdPConfig(config: Partial<IdPConfig>): void {
    ...
    if (config.type === IdPType.SAML) {
      if (!config.saml_config) {
        throw new IdPError(
          IdPErrorCode.INVALID_CONFIGURATION,
          'SAML configuration is required'
        );
      }
      ...
      if (!config.saml_config.entity_id || !config.saml_config.sso_url) {
        throw new IdPError(
          IdPErrorCode.INVALID_CONFIGURATION,
          'SAML entity_id and sso_url are required'
        );
      }
    }
    ...
    if (!config.attribute_mapping?.user_id || !config.attribute_mapping?.email) {
      throw new IdPError(
        IdPErrorCode.INVALID_CONFIGURATION,
        'Attribute mapping for user_id and email are required'
      );
    }
  }
   ...
   * Test IdP connection
   */
  async testIdPConnection(config: IdPConfig): Promise<{
    success: boolean;
    message: string;
    metadata?: any;
  }> {
    try {
      ...
      if (config.type === IdPType.OIDC && config.oidc_config) {
        ...
        const metadata = await this.oidcService.discoverConfiguration(
          config.oidc_config.issuer
        );
