...
export class AttributeMappingService {
  ...
  mapAttributes(config: IdPConfig, claims: Record<string, any>): Partial<IdPAuthResponse> {
    try {
      ...
      const mapped: Partial<IdPAuthResponse> = {
      ...
      if (!mapped.idp_user_id) {
        throw new IdPError(
          IdPErrorCode.ATTRIBUTE_MAPPING_FAILED,
          'Failed to map user_id from IdP claims',
          { mapping: mapping.user_id, claims }
        );
      }
      ...
      if (!mapped.email) {
        throw new IdPError(
          IdPErrorCode.ATTRIBUTE_MAPPING_FAILED,
          'Failed to map email from IdP claims',
          { mapping: mapping.email, claims }
        );
      }
      ...
      throw new IdPError(
        IdPErrorCode.ATTRIBUTE_MAPPING_FAILED,
        `Attribute mapping failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        { claims }
      );
    }
  }
   ...
   */
  private extractAttribute(claims: Record<string, any>, path: string): any {
    if (!path) {
      return undefined;
    }
   ...
   */
  private extractEmailVerified(claims: Record<string, any>): boolean {
  ...
  mapRoles(config: IdPConfig, claims: Record<string, any>): string[] {
    try {
      ...
      throw new IdPError(
        IdPErrorCode.ROLE_MAPPING_FAILED,
        `Role mapping failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        { claims }
      );
    }
  }
   ...
   */
  private extractRoles(claims: Record<string, any>, roleClaim: string): string[] {
