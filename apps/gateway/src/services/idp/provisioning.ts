...
export class JITProvisioningService {
  ...
  async provisionUser(
    config: IdPConfig,
    authResponse: Partial<IdPAuthResponse>
  ): Promise<ProvisioningResult> {
    if (!config.provisioning.enabled) {
      throw new IdPError(
        IdPErrorCode.PROVISIONING_FAILED,
        'JIT provisioning is not enabled for this IdP'
      );
    }

    try {
      // Check if user already exists
      const existingIdentity = await this.findFederatedIdentity(
        config.tenant_id,
        config.idp_id,
        authResponse.idp_user_id!
      );
  ...
  private async createUser(
    ...
    if (!config.provisioning.create_users) {
      throw new IdPError(
        IdPErrorCode.PROVISIONING_FAILED,
        'User creation is not enabled for this IdP'
      );
    }
  ...
  private async updateUser(
    config: IdPConfig,
    existingIdentity: FederatedIdentity,
    authResponse: Partial<IdPAuthResponse>
  ): Promise<ProvisioningResult> {
    ...
    const updatedIdentity: FederatedIdentity = {
      ...existingIdentity,
   ...
   */
  private async findFederatedIdentity(
    tenantId: string,
    idpId: string,
    idpUserId: string
   ...
   */
  private async storeFederatedIdentity(identity: FederatedIdentity): Promise<void> {
   ...
   */
  private async updateFederatedIdentity(identity: FederatedIdentity): Promise<void> {
  ...
  async bulkProvisionUsers(
    config: IdPConfig,
    users: Array<Partial<IdPAuthResponse>>
  ): Promise<{
    successful: number;
    failed: number;
    errors: Array<{ user: Partial<IdPAuthResponse>; error: string }>;
  }> {
    let successful = 0;
    let failed = 0;
    const errors: Array<{ user: Partial<IdPAuthResponse>; error: string }> = [];
