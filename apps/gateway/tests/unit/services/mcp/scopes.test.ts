...
describe('Scope-based Authorization System', () => {
  ...
  beforeEach(() => {
    ...
    scopeRegistry = new (ScopeRegistry as any)(); // Bypass private constructor for testing
    scopeValidator = new ScopeValidator(scopeRegistry);
    
    // Initialize default scopes by calling the private method
    (scopeRegistry as any).initializeDefaultScopes();
  });
  ...
  describe('ScopeValidator', () => {
    ...
    it('should fail validation when required scopes are missing', () => {
      const result = scopeValidator.validateScopes(
        ['mcp:tools:read', 'mcp:resources:write'],
        ['mcp:tools:read'] // Missing mcp:resources:write
      );
    ...
    it('should handle hierarchical scope inheritance (wildcard patterns)', () => {
      ...
      const result = scopeValidator.validateScopes(
        ['mcp:tools:read'],
        ['mcp:tools:*'] // Wildcard should satisfy specific scope
      );
    ...
    it('should validate multiple scopes with wildcard inheritance', () => {
      const result = scopeValidator.validateScopes(
        ['mcp:tools:read', 'mcp:resources:write'],
        ['mcp:tools:*', 'mcp:resources:write'] // Wildcard and exact match
      );
    ...
    it('should fail when wildcard does not match', () => {
      const result = scopeValidator.validateScopes(
        ['mcp:config:read'],
        ['mcp:tools:*'] // tools:* should not match config:read
      );
  ...
  describe('MCP Tool Invocation Validation', () => {
    it('should validate MCP tool invocation with required scopes', () => {
      const request: MCPTollInvocationRequest = {
        toolName: 'read_data',
        parameters: {},
      ...
      const result = scopeValidator.validateMCPToolInvocation(
        request,
    ...
    it('should fail MCP tool invocation when required permissions are missing', () => {
      ...
      const result = scopeValidator.validateMCPToolInvocation(
        request,
        ['mcp:tools:read'] // Token only has read, but tool needs write
      );
    ...
    it('should validate MCP tool invocation with wildcard scope inheritance', () => {
      ...
      const result = scopeValidator.validateMCPToolInvocation(
        request,
        ['mcp:tools:*'] // Wildcard should satisfy read
      );
  ...
  describe('Wildcard Pattern Matching', () => {
    it('should match specific scope to wildcard', () => {
      ...
      const matches = (scopeValidator as any).matchesWildcardPattern('mcp:tools:read', 'mcp:tools:*');
      expect(matches).toBe(true);
    });

    it('should not match unrelated scopes to wildcard', () => {
      const matches = (scopeValidator as any).matchesWildcardPattern('mcp:config:read', 'mcp:tools:*');
      expect(matches).toBe(false);
    });

    it('should handle nested wildcard patterns', () => {
      const matches = (scopeValidator as any).matchesWildcardPattern('mcp:tools:admin:disable', 'mcp:tools:*');
      expect(matches).toBe(true);
    });

    it('should not match when token scope is not a wildcard', () => {
      const matches = (scopeValidator as any).matchesWildcardPattern('mcp:tools:read', 'mcp:tools:write');
      expect(matches).toBe(false);
    });
  });
