...
describe('Session Lifecycle Tests', () => {
  ...
  describe('Session Validation', () => {
    ...
    it('should update last accessed time on validation', async () => {
      ...
      expect(result.valid).toBe(true);
      expect(result.session!.last_accessed_at.getTime()).toBeGreaterThan(
  ...
  describe('Idle Timeout', () => {
    ...
    it('should extend idle timeout on activity', async () => {
      ...
      expect(result.valid).toBe(true);
      expect(result.session!.idle_timeout_at.getTime()).toBeGreaterThan(
        originalIdleTimeout.getTime()
      );
    });
  });
