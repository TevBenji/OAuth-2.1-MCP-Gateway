...
export class BillingService {
  ...
  private billingTiers: Record<string, BillingTierConfig>;
   ...
   */
  async calculateBillingCost(tenantId: string): Promise<number> {
    const billing = await this.usageService.getTenantBilling(tenantId);
    if (!billing) {
      return 0;
    }
  ...
  async checkAndSendUsageAlerts(): Promise<void> {
    ...
    for (const row of result.results) {
      ...
      const billingTier = row.billing_tier as string;
  ...
  async processBillingPeriodEnd(): Promise<void> {
    ...
    for (const row of result.results) {
      const tenantId = row.tenant_id as string;
      const currentPeriodEnd = new Date(row.current_period_end as string);

      // Only process if the period ended today
      const today = new Date();
      if (
        currentPeriodEnd.getDate() === today.getDate() &&
