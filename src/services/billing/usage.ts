...
export class UsageTrackingService {
  ...
  async getHistoricalUsage(
    ...
    return result.results as UsageRecord[];
  }
  ...
  async getUsageAlerts(tenantId: string): Promise<UsageAlert[]> {
    ...
    return result.results as UsageAlert[];
  }

  /**
   * Creates a usage alert for a tenant
   */
  async createUsageAlert(alert: Omit<UsageAlert, 'id' | 'triggered_at' | 'notification_sent'>): Promise<UsageAlert> {
    const id = crypto.randomUUID();
