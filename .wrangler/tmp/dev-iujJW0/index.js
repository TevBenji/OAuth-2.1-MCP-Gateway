...
// .wrangler/tmp/bundle-8slobd/checked-fetch.js
...
// .wrangler/tmp/bundle-8slobd/strip-cf-connecting-ip-header.js
...
// src/types/oauth.ts
var AuthorizeRequestSchema = external_exports.object({
  client_id: external_exports.string().min(1),
  redirect_uri: external_exports.string().url(),
  response_type: external_exports.literal("code"),
  scope: external_exports.string().optional(),
  state: external_exports.string().min(1),
  // PKCE parameters (RFC 7636) - mandatory in OAuth 2.1
  code_challenge: external_exports.string().min(43).max(128),
  code_challenge_method: external_exports.literal("S256"),
  // Resource Indicators (RFC 8707)
  resource: external_exports.string().url().optional()
});
var TokenRequestSchema = external_exports.object({
  grant_type: external_exports.enum(["authorization_code", "refresh_token"]),
  // Authorization Code Grant
  code: external_exports.string().optional(),
  redirect_uri: external_exports.string().url().optional(),
  client_id: external_exports.string().min(1),
  // PKCE verifier (RFC 7636)
  code_verifier: external_exports.string().min(43).max(128).optional(),
  // Resource Indicators (RFC 8707)
  resource: external_exports.string().url().optional(),
  // Refresh Token Grant
...
var ClientRegistrationRequestSchema = external_exports.object({
  redirect_uris: external_exports.array(external_exports.string().url()).min(1),
  client_name: external_exports.string().optional(),
  client_uri: external_exports.string().url().optional(),
  logo_uri: external_exports.string().url().optional(),
  scope: external_exports.string().optional(),
  contacts: external_exports.array(external_exports.string().email()).optional(),
  tos_uri: external_exports.string().url().optional(),
  policy_uri: external_exports.string().url().optional(),
  token_endpoint_auth_method: external_exports.enum(["none", "client_secret_post", "client_secret_basic"]).optional(),
  grant_types: external_exports.array(external_exports.enum(["authorization_code", "refresh_token"])).optional(),
  response_types: external_exports.array(external_exports.literal("code")).optional()
});
...
var ClientService = class {
  ...
  async registerClient(request, tenantId) {
    ...
    const clientSecret = authMethod !== "none" ? this.generateClientSecret() : void 0;
    ...
    const clientSecretExpiresAt = clientSecret ? now + 365 * 24 * 60 * 60 : void 0;
    const clientData = {
      client_id: clientId,
  ...
  async getClient(clientId, tenantId) {
    ...
    return {
      ...
      contacts: result.contacts ? JSON.parse(result.contacts) : void 0
    };
  }
  /**
  ...
  async deleteClient(clientId, tenantId) {
    const result = await this.db.prepare(`DELETE FROM ${DATABASE_CONSTANTS.TABLES.OAUTH_CLIENTS} WHERE client_id = ? AND tenant_id = ?`).bind(clientId, tenantId).run();
...
__name(ClientService, "ClientService");

// src/handlers/oauth/register.ts
var registerClient = /* @__PURE__ */ __name(async (c) => {
  try {
    let body;
    try {
...
function validateClientRegistrationRequest(request) {
...
__name(validateClientRegistrationRequest, "validateClientRegistrationRequest");
...
function isValidScopeString(scope) {
...
__name(isValidScopeString, "isValidScopeString");
...
app.post("/tenants/:id/clients", async (c) => {
  ...
  try {
    ...
    const clientSecret = clientData.client_type === "confidential" ? `secret_${crypto.randomUUID()}` : void 0;
    await c.env.DB.prepare(
...
// src/handlers/admin/ui.ts
var app2 = new Hono2();
app2.get("/admin", async (c) => {
  const html = `
    <!DOCTYPE html>
    <html lang="en">
      <head>
...
app3.get("/.well-known/oauth-authorization-server", (c) => {
...
app3.post("/register", registerClient);
app3.route("/admin/api", api_default);
app3.route("/admin", ui_default);
app3.notFound((c) => {
...
// node_modules/.pnpm/wrangler@3.114.14_@cloudflare+workers-types@4.20250927.0/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// node_modules/.pnpm/wrangler@3.114.14_@cloudflare+workers-types@4.20250927.0/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-8slobd/middleware-insertion-facade.js
...
// node_modules/.pnpm/wrangler@3.114.14_@cloudflare+workers-types@4.20250927.0/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-8slobd/middleware-loader.entry.ts
